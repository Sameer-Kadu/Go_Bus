...
const TicketSummary = () => {
  ...
  const handlePayment = async () => {
    setIsLoading(true); // Show loader when payment starts
    ...
    setIsLoading(false); // Hide loader after payment process
  };

  const formatDeparture = (date, time) => {
    const combined = moment(`${date} ${time}`, 'YYYY-MM-DD HH:mm');
    return combined.format('DD/MMM/YYYY HH:mm');
  };
  ...
  return (
    <div className="max-w-2xl mt-10 mx-auto p-4 bg-white rounded-lg shadow">
      ...
      <div className="mb-6">
        <h1 className="text-yellow-800 text-xl font-semibold">{bus.agencyName}</h1>
          ...
          <div className="flex-1">
            <div className="flex justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-600">{bus.boardingPoint}</p>
              </div>
      ...
      <div className="mt-4 flex justify-center">
        <Button
          ...
          leftIcon={!isLoading && <TiLocationArrow />}
          change={handlePayment}
          containerClass={`!bg-yellow-300 flex-center gap-1 ${isLoading ? "opacity-50 cursor-not-allowed" : ""}`}
          disabled={isLoading}
        />
      </div>
